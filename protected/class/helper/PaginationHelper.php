...
.inactivePrev, .inactiveNext{
     ...
     */
    function __construct($baseURL = '', $requestParams = array(), $totalItems = 120, $itemPerPage = 10, $maxlength = 10, $prevText = '&laquo;', $nextText = '&raquo;') {

        if (isset($requestParams) && is_array($requestParams)) {
    ...
    public function paginate($page, $itemPerPage = 0) {
        ...
        if ($this->_noNextPrev)
            $this->components['prev_link'] = ($this->currentPage != 1 && $this->totalItem >= $this->maxLength) ? "<a class=\"{$this->prevCss}\" href=\"{$this->_link}pindex={$prev_page}\">{$this->prevText}</a> " : "<span class=\"{$this->inactivePrevCss}\">{$this->prevText}</span> ";
        else
            $this->output = ($this->currentPage != 1 && $this->totalItem >= $this->maxLength) ? "<a class=\"{$this->prevCss}\" href=\"{$this->_link}pindex={$prev_page}\">{$this->prevText}</a> " : "<span class=\"{$this->inactivePrevCss}\">{$this->prevText}</span> ";
        ...
        if ($this->totalPage > $this->maxLength) {
            ...
            for ($i = 1; $i <= $this->totalPage; $i++) {
                ...
                if ($i == 1 || $i == $this->totalPage || in_array($i, $range)) {
                    ...
                    $this->output .= ($i == $this->currentPage) ? "<a class =\"{$this->currentCss}\" href=\"javascript:void(0);\">$i" : "<a class=\"{$this->pagesCss}\"  href=\"{$this->_link}pindex={$i}\">$lastDot $i";
            ...
            if ($this->_noNextPrev)
                $this->components['next_link'] = ($this->currentPage != $this->totalPage && $this->totalItem >= $this->maxLength) ? "<a class=\"{$this->nextCss}\" href=\"{$this->_link}pindex={$next_page}\">{$this->nextText}</a>\n" : "<span class=\"{$this->inactiveNextCss}\">{$this->nextText}</span>\n";
            else
                $this->output .= ($this->currentPage != $this->totalPage && $this->totalItem >= $this->maxLength) ? "<a class=\"{$this->nextCss}\" href=\"{$this->_link}pindex={$next_page}\">{$this->nextText}</a>\n" : "<span class=\"{$this->inactiveNextCss}\">{$this->nextText}</span>\n";
        }
        else {
            for ($i = 1; $i <= $this->totalPage; $i++) {
                $this->output .= ($i == $this->currentPage) ? "<a class=\"{$this->currentCss}\" href=\"javascript:void(0);\">$i</a> " : "<a class=\"{$this->pagesCss}\" href=\"{$this->_link}pindex=$i\">$i</a> ";
            }
    ...
    public function showPageSize($sizeArr = null) {
        ...
        return "<select class=\"{$this->pageSizeCss}\" onchange=\"window.location='{$this->_link}pindex=1/'+this[this.selectedIndex].value; return false\">$items</select>\n";
    }
    ...
    public function showJumpMenu($withPageSize = false) {
        ...
        if ($withPageSize)
            return "<select class=\"{$this->dropDownCss}\" onchange=\"window.location='{$this->_link}pindex='+this[this.selectedIndex].value+'/$this->itemPerPage';return false\">$option</select>\n";
        return "<select class=\"{$this->dropDownCss}\" onchange=\"window.location='{$this->_link}pindex='+this[this.selectedIndex].value;return false\">$option</select>\n";
    }
